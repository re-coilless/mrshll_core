...
function mrshll.play_sound( event )
...
function mrshll.new_button( pic_x, pic_y, pic_z, pic, data )
	data = data or {}
...
mrshll.is_showing = mrshll.is_showing or false
mrshll.gonna_play = mrshll.gonna_play or ( is_playing == 0 and mrshll.last_played == nil and pen.setting_get( "mrshll_core.AUTOPLAY" ))
mrshll.num_override, mrshll.last_played = mrshll.num_override or 0, mrshll.last_played or 0
mrshll.switch_delay = mrshll.switch_delay or ( mrshll.gonna_play and delay or 0 )
mrshll.list = dofile( "mods/mrshll_core/mrshll_list.lua" )
if( not( pen.vld( mrshll.filter_list ))) then
	mrshll.filter_list = {}
	for cat,_ in pen.t.order( mrshll.list ) do table.insert( mrshll.filter_list, cat ) end
end


local function reset()
	ComponentSetValue2( storage_delay, "value_float", 0 )
	mrshll.switch_delay, mrshll.gonna_play, mrshll.last_played = delay, false, 0
	pen.magic_storage( entity_id, "playlist", "value_string", "" )
end

if( not( pen.vld( this_ordered ))) then
	this_ordered = {}
	for cat,v in pen.t.order( mrshll.list ) do
...
local goners_order = {}
local goners_ignore = {}
if( mrshll.playlist_update ) then
	mrshll.playlist_update = false
	pen.t.loop( this_ordered, function( i, v )
		local is_nuked = this_ignored[ v[2]] ~= nil
		local is_real = pen.vld( pen.t.get( mrshll.list[ v[1]], v[2], nil, nil, {}))
...
elseif( mrshll.playlist_update == nil ) then
	mrshll.playlist_update = false
	pen.t.loop( ordered, function( k, o )
		...
		pen.t.loop( ignored[k], function( i,_ )
			if( pen.t.loop( mrshll.list, function( _,v )
	...
	if( is_playing == 0 ) then
		...
		if( mrshll.switch_delay == 0 ) then
			...
			pen.hallway( function()
				...
				local is_new = played_list[ this_ordered[ num ][2]] == nil
				local is_cat = mrshll.filtering == 0 or this_ordered[ num ][1] == mrshll.filter_list[ mrshll.filtering ]
				if( is_new and is_cat ) then return end
				
				local is_done = true
				for i,v in ipairs( this_ordered ) do
					is_new = played_list[ v[2]] == nil
					is_cat = mrshll.filtering == 0 or v[1] == mrshll.filter_list[ mrshll.filtering ]
					if( is_new and is_cat ) then is_done, num = false, i; break end
				end

				if( is_done ) then played_list = {} else return end
				if( mrshll.filtering > 0 ) then num = mrshll.last_played; return end
				if( mrshll.last_played == num ) then num = num < 2 and #this_ordered or mrshll.last_played - 1 end
			end)
			force_the_same, mrshll.last_played, played_list[ this_ordered[ num ][2]] = false, num, 1
			ComponentSetValue2( storage_played, "value_string", pen.t.pack( pen.t.unarray( played_list )))
			
			local v = this_ordered[ num ]
			local song = pen.t.get( mrshll.list[ v[1]], v[2])
			...
			mrshll.switch_delay = mrshll.switch_delay - 1
			if( mrshll.switch_delay == 5 ) then
				pen.t.loop({"left","right"}, function( i, v )
...
GlobalsSetValue( "MRSHLL_OST_VOLUME_", pen.estimate(
	"mrshll_ost_volume_fade", { mrshll.gonna_play and 0 or 1, 1 }, { "exp", 0.02 }))
...
if( is_going and not( pen.is_inv_active( hooman ))) then
	if( not( no_dragger )) then
		...
		if( r_clicked ) then
			mrshll.play_sound( "ass/special_button" )
			mrshll.is_moving = not( mrshll.is_moving )
		end
	end
	...
	if( mrshll.is_listing ) then
			...
			pen.t.loop( mrshll.is_showing and this_ignored or this_ordered, function( i, v )
				...
				if( mrshll.is_showing ) then
					cat, song = pen.cache({ "mrshll_deleted", i }, function()
						return unpack( pen.t.loop( mrshll.list, function( cat,v )
				...
				if( mrshll.is_showing or #this_ordered > 1 ) then
					...
					if( r_clicked ) then
						...
						ignored[ playlist_num ] = this_ignored
						pen.setting_set( "mrshll_core.IGNORE_LIST", pen.t.parse( ignored ))
						mrshll.playlist_update = not( mrshll.is_showing )
						reset()
					end
				end
				...
				if( not( mrshll.is_showing )) then
					if( clicked ) then
						...
						force_the_same, mrshll.num_override = true, i
						ComponentSetValue2( storage_delay, "value_float", 0 )
		...
		if( clicked or r_clicked ) then
			...
			else mrshll.filtering = mrshll.filtering < 1 and #mrshll.filter_list or mrshll.filtering - 1 end
			mrshll.play_sound( "ass/special_button" )
		end
